...
def test():
    ...
    assert equal(
        m3.inverse(), m3_inv
    ), """Error in your inverse function for the 1 x 1 case"""
    assert equal(
        m1_x_m2.inverse(), m1_m2_inv
    ), """Error in your inverse function for the first 2 x 2 case"""
    assert equal(
        I2.inverse(), I2
    ), """Error in your inverse function for the second 2 x 2 case"""
    ...
    assert equal(
        top_ones - left_ones.T(), m.zeroes(2, 2)
    ...
    assert type(I2 + I2_neg) == type(
        zero
    ), "Error: Your __add__ function does not return a Matrix"
    assert type(m1 * m2) == type(
        m1_x_m2
    ), "Error: Your __mul__ function does not return a Matrix"
    ...
    assert type(top_ones - left_ones.T()) == type(
        m.zeroes(2, 2)
